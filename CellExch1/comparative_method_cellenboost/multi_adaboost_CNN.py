...
class AdaBoostClassifier(object):
    ...
    def _samme_proba(self, estimator, n_classes, X):
        ...
        """
        proba = estimator.predict(X)  #estimator.predict_proba
...
#########################################################        
        ...
        y_predict_proba = estimator.predict(X)  # estimator.predict_proba(X)
...
###########################################################
    ...
    def predict_proba(self, X):
        ...
        if self.algorithm_ == 'SAMME.R':
            ...
            proba = sum(estimator.predict(X) * w  # estimator.predict_proba(X) * w
                        for estimator, w in zip(self.estimators_,
                                                self.estimator_weights_))
