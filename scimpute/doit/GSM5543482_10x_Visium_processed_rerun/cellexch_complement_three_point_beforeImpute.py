...
folders_to_create = [
    r"/home/jby2/XH/scImpute/doit/GSM5543482_10x_Visium_processed_rerun/beforeImpute/expression_thresholding",
    r"/home/jby2/XH/scImpute/doit/GSM5543482_10x_Visium_processed_rerun/beforeImpute/expression_product",
    r"/home/jby2/XH/scImpute/doit/GSM5543482_10x_Visium_processed_rerun/beforeImpute/cell_expression",
    r"/home/jby2/XH/scImpute/doit/GSM5543482_10x_Visium_processed_rerun/beforeImpute/Three",
    r"/home/jby2/XH/scImpute/doit/GSM5543482_10x_Visium_processed_rerun/beforeImpute/Three/TOP",
    r"/home/jby2/XH/scImpute/doit/GSM5543482_10x_Visium_processed_rerun/beforeImpute/"]
for folder in folders_to_create:
    os.makedirs(folder, exist_ok=True)
cancer = r'/home/jby2/XH/scImpute/doit/GSM5543482_10x_Visium_processed_rerun/beforeImpute/'  # File directory for species
# x_ytick = ['Astro', 'Neu', 'Olfactory', 'Stem', 'cluster 4', 'cluster 5', 'cluster 6', 'cluster 7', 'cluster 8', 'cluster 9', 'cluster 10', 'cluster 11', 'cluster 12', 'cluster 13', 'cluster 14', 'cluster 15', 'cluster 16', 'cluster 17', 'cluster 18']
x_ytick = ['Medullary', 'Mesangial', 'Collecting', 'Tubule', 'Progenitor']
cell_type = 4
thr = pd.read_csv("/home/jby2/XH/scImpute/doit/GSM5543482_10x_Visium_processed_rerun/beforeImpute/The_expression_thresholding_data.csv", header=None, index_col=None).to_numpy()
pro = pd.read_csv("/home/jby2/XH/scImpute/doit/GSM5543482_10x_Visium_processed_rerun/beforeImpute/The_expression_product_data.csv", header=None, index_col=None).to_numpy()
cell = pd.read_csv("/home/jby2/XH/scImpute/doit/GSM5543482_10x_Visium_processed_rerun/beforeImpute/The_cell_expression_data.csv", header=None, index_col=None).to_numpy()
LRI = pd.read_csv("/home/jby2/XH/Cell_Dialog/mouse_brain/LRI.csv", header=None, index_col=None).to_numpy()  # LRI obtained after filtering
...
for j in range(len(data)):
    data11 = data[j][0]
    last_underscore_index = data11.rfind('_')
    data1 = float(data11[last_underscore_index + 1:])
    first_underscore_index = data11.find('_')
    a1 = data11[:first_underscore_index]
    if len(a1) % 2 == 0:
        if a1[0:len(a1)//2] == a1[len(a1)//2:len(a1)]:
            data1 = 0.0
    data2.append(data1)
# normalization
_range = np.max(data2) - np.min(data2)
if _range == 0:
    aaa = (data2 - np.min(data2)) / (_range + 1)
else:
...
a = df.shape[0]
node_colors = [
    'hotpink', 'darkorange', 'blue', 'yellow', 'magenta', 'cyan',
    'gray', 'violet', 'red', 'khaki', 'darkred', 'saddlebrown',
    'lime', 'indigo', 'teal', 'gold', 'forestgreen', 'dodgerblue',
    'orangered', 'slateblue', 'lightcoral', 'darkturquoise'
][:a]
G = nx.DiGraph()
...
# plt.show()
# print(' Time: {}seconds, mem: {}MB'.format((time() - itime), psutil.Process(os.getpid()).memory_info().rss / 1024 / 1024))