...
def parse_args():
    ...
    parser.add_argument('-f', '--feature', type=str, default='adj',
                        help='Features used in the first step')
    parser.add_argument('-sp', '--save-path', type=str, default='',
                        help='Path to save checkpoints. Default is ./checkpoints/model_name.')
    parser.add_argument('-lp', '--load-path', type=str, default='',
                        help='Path to save checkpoints. Default is ./data.')
    ...
    if args.save_path:
        ...
        args.save_path = os.path.join('/home/jby2/XH/CellExch/mouse_brain/other_tools/scHyper/checkpoints/', args.data, args.model_name)
...
def main():
    ...
    def save_embeddings(model, origin=False):
        ...
        with torch.no_grad():
            ...
            for i in range(len(num_list)):
                ...
                np.save("/home/jby2/XH/CellExch/mouse_brain/other_tools/scHyper/mymodel_%d.npy" % (i), static)

                if origin:
                    np.save("/home/jby2/XH/CellExch/mouse_brain/other_tools/scHyper/mymodel_%d_origin.npy" % (i), static)
    ...
    def train_batch_hyperedge(model, loss_func, batch_data, batch_weight, type, y=""):
        ...
        if len(y) == 0:
            x, y, w = generate_negative(x, "train_dict", type, w) # 为训练集生成负样本
    ...
    def train(args, model, loss, training_data, validation_data, optimizer, epochs, batch_size, only_rw):
        ...
        for epoch_i in range(epochs):
            ...
            bce_loss, skipgram_loss, recon_loss, train_accu, auc1, auc2 = train_epoch(
                args, model, loss, training_data, optimizer, batch_size, only_rw, train_type) # 添加了y, pred
    ...
    def generate_negative(x, dict1, get_type='all', weight="", forward=True):
            ...
            for i in range(neg_num):
                ...
                while not a.isdisjoint(dict1):
                    ...
                    if trial >= 1000:
                        temp = ""
                        break
                    change = np.random.randint(0, sample.shape[-1], 1)[0]
                    if trial < 999:
        ...
        # new_x = x[new_index]
        if new_index.size == 0:
            raise ValueError("new_index is invalid")
        new_x = [x[i] for i in new_index]
        ...
        return torch.cat([x, neg]), torch.cat(
            ...
            ((torch.ones((len(x), 1), device=device) * new_weight.view(-1, 1),
    ...
    #use_to_predict_zip = np.load("../data/%s/use_to_predict.npz" % (args.data), allow_pickle=True)

    train_zip = np.load(os.path.join(args.load_path, "%s/train_data.npz" % (args.data)), allow_pickle=True)
    test_zip = np.load(os.path.join(args.load_path,"%s/test_data.npz" % (args.data)), allow_pickle=True)
    use_to_predict_zip = np.load(os.path.join(args.load_path,"%s/use_to_predict.npz" % (args.data)), allow_pickle=True)
    train_data, test_data, use_to_predict = train_zip['train_data'], test_zip['test_data'], use_to_predict_zip['use_to_predict']
    ...
    if len(num) > 1:
        for i in range(len(node_type_mapping) - 1):
            ...
            use_to_predict[:, i + 1] += num_list[node_type_mapping[i + 1] - 1]
    ...
    use_to_predict = add_padding_idx(use_to_predict)
    ...
    train(args, (classifier_model, Randomwalk_Word2vec),
          ...
          optimizer=[optimizer], epochs=20, batch_size=batch_size, only_rw=False)


    checkpoint = torch.load(os.path.join(args.save_path, 'model.chkpt'))
    classifier_model1 = Classifier(
        n_head=8,
        d_model=args.dimensions,
        d_k=16,
        d_v=16,
    ...
    classifier_model1.load_state_dict(checkpoint['model_link'])
    classifier_model1.eval()

    use_to_predict = torch.from_numpy(use_to_predict).long()
    use_to_predict = use_to_predict.to('cuda:0')

    batchsize = 32
    with torch.no_grad():
        predictions_train_data = []
        for i in range(0, len(use_to_predict), batchsize):
            batchdata = use_to_predict[i:i + batchsize]
            batch_predictions = classifier_model1(batchdata)
            predictions_train_data.append(batch_predictions)
            del batchdata
        predictions_train_data = torch.cat(predictions_train_data, dim=0)
    with open('/home/jby2/XH/CellExch/mouse_brain/other_tools/scHyper/predictions.pkl', 'wb') as f:
        pickle.dump(predictions_train_data, f)
